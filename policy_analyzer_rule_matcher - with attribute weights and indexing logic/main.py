...
def load_rules_database(csv_path: str, config: dict) -> dict:
    """
    Loads and groups the rules CSV data.
    If prefiltering is enabled, it builds an indexed/nested dictionary.
    Otherwise, it returns a flat dictionary of rules.
    """
    ...
    enable_filtering = cfg_logic.get("ENABLE_PREFILTERING", False)
    index_attribute = cfg_logic.get("PREFILTER_INDEX_ATTRIBUTE")
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if rule_id not in rules_db_flat:
                rules_db_flat[rule_id] = {}

            rules_db_flat[rule_id][attr_path] = attr_value

        # --- New Logic: Index the flat DB if enabled ---
        if enable_filtering and index_attribute:
            print(f"Prefiltering enabled. Indexing rules on: {index_attribute}...")
            rules_db_indexed = {}
            unindexed_count = 0

            for rule_id, rule_map in rules_db_flat.items():
                # Get the value to index on (e.g., "Reimbursement Policy")
                index_key_value_raw = rule_map.get(index_attribute)

                # --- FIX: Normalize the index key ---
                index_key_value = string_similarity.normalize(index_key_value_raw)

                # Use a placeholder if the rule is missing this attribute
                if not index_key_value:
                    index_key_value = "_UNINDEXED_"
                    unindexed_count += 1

                if index_key_value not in rules_db_indexed:
                    rules_db_indexed[index_key_value] = {}

                rules_db_indexed[index_key_value][rule_id] = rule_map

            print(
                f"Successfully loaded and indexed {len(rules_db_flat)} unique rules into {len(rules_db_indexed)} groups.")
            if unindexed_count > 0:
                print(f"Warning: {unindexed_count} rules could not be indexed (missing {index_attribute}).")
            return rules_db_indexed
        else:
            # Return the old flat structure
            print(f"Successfully loaded {len(rules_db_flat)} unique rules (Prefiltering disabled).")
            return rules_db_flat
...
def compare_policy_to_rules(policy_map: dict, policy_filename: str, rules_db: dict, config: dict) -> (dict, list):
    ...
    cfg_weights = config.get("ATTRIBUTE_WEIGHTS", {})

    # --- New: Check if weighting is enabled ---
    enable_weighting = cfg_logic.get("ENABLE_WEIGHTING", False)

    default_weight = cfg_weights.get("_default_weight", 1)
    ...
    for rule_id, rule_map in rules_db.items():
        ...
        numerator_score = 0.0
        denominator_score = 0.0

        # --- Old Count-Based Logic (used if weighting is off) ---
        ...
        for key in count1_keys:
            ...
            weight = cfg_weights.get(key, default_weight)
            if enable_weighting:
                denominator_score += weight
        ...
        for key in count2_keys:
            if enable_weighting:
                # Add this attribute's weight to the total possible score
                denominator_score += cfg_weights.get(key, default_weight)
        ...
        if enable_weighting:
            ...
            # Use original count-based formula
            denominator_count = count1 + count2 + count3
            if denominator_count > 0:
                score = (count1_2 + count1_3) / denominator_count
...
def analyze_scores_and_decide(policy_map: dict, rules_db: dict, scores: dict, config: dict) -> (str, str, float, list):
    ...
    if max_score >= main_threshold:
        ...
        # Note: rules_db is flat here, which is fine for .get()
...
def main():
    ...
    cfg_logic = config.get("LOGIC_CONTROLS", {})  # Get logic config
    ...
    rules_db_full = load_rules_database(paths.get("RULES_CSV_PATH"), config)
    if not rules_db_full:
    ...
    for policy_filename in policy_files:
        ...
        try:
            ...
            # --- New Logic: Select which rules to compare against ---
            rules_to_compare = {}
            is_filtered = False
            if cfg_logic.get("ENABLE_PREFILTERING", False):
                index_attr = cfg_logic.get("PREFILTER_INDEX_ATTRIBUTE")
                if index_attr:
                    is_filtered = True
                    # Get the policy's value for the index attribute
                    policy_index_value_raw = policy_map.get(index_attr)

                    # --- FIX: Normalize the policy's lookup key ---
                    policy_index_value = string_similarity.normalize(policy_index_value_raw)

                    if not policy_index_value:
                        policy_index_value = "_UNINDEXED_"

                    # Get the group of rules matching that index value
                    rules_to_compare = rules_db_full.get(policy_index_value, {})

                    # Also include any unindexed rules in the comparison
                    rules_to_compare.update(rules_db_full.get("_UNINDEXED_", {}))
            else:
                # If filtering is off, compare against all rules
                rules_to_compare = rules_db_full

            # Compare policy to the selected rules and get trace data
            scores, trace_data = compare_policy_to_rules(policy_map, policy_filename, rules_to_compare, config)

            all_trace_data.extend(trace_data)  # <-- Add to consolidated list

            # --- We need the *full* flat DB for the payor mismatch check ---
            # This is a bit inefficient, but needed if filtering is on
            # We can optimize this later if needed
            rules_db_flat_for_analysis = {}
            if is_filtered:
                for group in rules_db_full.values():
                    rules_db_flat_for_analysis.update(group)
            else:
                rules_db_flat_for_analysis = rules_db_full

            # Get final decision and Top N list
            decision, best_rule_id, max_score, top_n_matches = analyze_scores_and_decide(
                policy_map, rules_db_flat_for_analysis, scores, config
            )

            # --- Save Individual Trace CSV ---
            # --- FIX: Changed logging logic to always report the decision ---
            print(
            ...
            if trace_data:
                ...
                print(f"    -> No comparisons made (no matching rules found by pre-filter).")
            # --- END FIX ---
